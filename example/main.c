...
struct Display *eink_init() {
  ...
  gDisplay.width_bytes = (gDisplay.width_px % 8 == 0)
                             ? (gDisplay.width_px / 8)
                             : (gDisplay.width_px / 8 + 1);
  return &gDisplay;
}
...
void eink_clear_white() { eink_pattern(&gDisplay, 0xFF); }

void eink_clear_black() { eink_pattern(&gDisplay, 0x00); }
...
void pat_impl(uint8_t cmd) {
  ...
  // * Last drawable colum seems to be 237, even though display says 250. I'm
  // sure this is a bug in this code
...
void eink_draw_img(const char *path) {
  ...
  // Why flash? Paint_NewImage(BlackImage, EPD_2in13_V4_WIDTH,
  // EPD_2in13_V4_HEIGHT, 90, WHITE); Why flash? Paint_SelectImage(BlackImage);
...
void save_as_bmp(const char *filename, cairo_surface_t *surface) {
  ...
  int row_size =
      ((width + 31) / 32) * 4; // Each row must be a multiple of 4 bytes

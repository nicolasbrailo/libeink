...
static int xSpiXfer(int fd, int speed, const char *txBuf, char *rxBuf,
                    int count) {
...
int lgSpiOpen(int spiDev, int spiChan, int baud, int spiFlags) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "spiDev=%d spiChan=%d baud=%d spiFlags=0x%X", spiDev,
  ...
  handle = lgHdlAlloc(LG_HDL_TYPE_SPI, sizeof(lgSpiObj_t), (void **)&spi,
                      _lgSpiClose);

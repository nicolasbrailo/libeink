...
void emitNotifications(int count) {
  ...
  if (handles == NULL)
    handles = malloc(sizeof(int) * maxHandles);
  ...
  for (i = 0; i < numHandles; i++) {
    status =
        lgHdlGetLockedObjTrusted(handles[i], LG_HDL_TYPE_NOTIFY, (void **)&h);
    ...
    if (h->state == LG_NOTIFY_CLOSING) {
      ...
      if (emit) {
        ...
        while (emit > 0) {
          if (emit > max_emits) {
            err =
                write(h->fd, report + sent, max_emits * sizeof(lgGpioReport_t));
            ...
            if (err != (max_emits * sizeof(lgGpioReport_t))) {
              if (err < 0) {
                if ((errno != EAGAIN) && (errno != EWOULDBLOCK)) {
                  ...
                  LG_DBG(LG_DEBUG_ALWAYS, "fd=%d err=%d errno=%d", h->fd, err,
                         errno);
            ...
            err = write(h->fd, report + sent, emit * sizeof(lgGpioReport_t));
...
int emit(int count, uint64_t tmax) {
  ...
  for (i = 0; i < count; i++) {
    if (aBuf[i].report.timestamp > tmax)
      break;
  }
...
void printbuf(int count, char *str) {
  ...
  for (i = 0; i < count; i++) {
    fprintf(stderr, "%" PRIu64 " %d %d %d (%d of %d)\n",
            aBuf[i].report.timestamp, aBuf[i].report.level, aBuf[i].report.chip,
            aBuf[i].report.gpio, i + 1, count);
  }
}
...
void lgcheck(int count, char *str) {
  ...
  for (i = 1; i < count; i++) {
    ...
    if (diff > 0) {
      fprintf(stderr, "%s\n%" PRIu64 " %d %d %d\n", str,
              aBuf[i - 1].report.timestamp, aBuf[i - 1].report.level,
              aBuf[i - 1].report.chip, aBuf[i - 1].report.gpio);
      fprintf(stderr, "%" PRIu64 " %d %d %d\n\n", aBuf[i].report.timestamp,
              aBuf[i].report.level, aBuf[i].report.chip, aBuf[i].report.gpio);
    }
  }
}
  ...
  if (p->debounce_nanos && !p->debounced) {
    ...
    */
    ...
    if (nano_diff > p->debounce_nanos) {
      ...
      if ((p->eFlags != LG_BOTH_EDGES) || (p->last_rpt_lv != p->last_evt_lv)) {
        /*
        LG_DBG(LG_DEBUG_ALWAYS, "g=%d(%d) diff=%"PRId64" deb=%"PRIu64"
        ts=%"PRIu64" lts=%"PRIu64"", p->gpio, p->last_evt_lv, nano_diff,
        p->debounce_nanos, ts/100000, p->last_evt_ts/100000);
        */
  ...
  if (p->watchdog_nanos && !p->watchdogd) {
    ...
    */
    ...
    if (nano_diff > p->watchdog_nanos) {
      /*
      LG_DBG(LG_DEBUG_ALWAYS, "g=%d(2) diff=%"PRId64" wdg=%"PRIu64" ts=%"PRIu64"
      lts=%"PRIu64"", p->gpio, nano_diff, p->watchdog_nanos, ts/100000,
      p->last_rpt_ts/100000);
      */
        ...
        for (i = 0; i < num_gpio; i++) {
          ...
          if ((retval > 0) && (pfd[i].revents)) {
            ...
            if (bytes > 0) {
              ...
              if (bytes) {
                if (p->active)
                  LG_DBG(LG_DEBUG_ALWAYS, "bytes left=%d (%s)", bytes,
                         strerror(errno));
              }
            } else {
              if (p->active)
                LG_DBG(LG_DEBUG_ALWAYS, "read error %d (%s)", errno,
                       strerror(errno));
            }
          }
          ...
          if (gpiobasecount < count) {
            if (p->state->alertFunc) {
              (p->state->alertFunc)(count - gpiobasecount, &aBuf[gpiobasecount],
                                    p->state->userdata);
            }
          }
        }
...
void lgPthAlertStop(lgChipObj_p chip) {
  ...
  for (evt = alertRec; evt != NULL; evt = evt->next) {
    if (chip->handle == evt->chip->handle)
      evt->active = 0;
  }

...
static void xCreatePipe(const char *name, int perm) {
  ...
  if (chmod(name, perm) < 0) {
    LG_DBG(LG_DEBUG_ALWAYS, "Can't set permissions (%d) for %s, %m", perm,
           name);
    return;
  }
}
...
static void _notifyClose(lgNotify_t *h) {
  ...
  LG_DBG(LG_DEBUG_INTERNAL, "fd=%d pipe_no=%d objp=*%p", h->fd, h->pipe_number,
         h);
...
void lgNotifyCloseOrphans(int slot, int fd) {
  ...
  for (i = 0; i < numHandles; i++) {
    ...
    if ((handles[i] != slot) && (status >= 0) &&
        (h->state >= LG_NOTIFY_RUNNING) && (h->fd == fd)) {
      LG_DBG(LG_DEBUG_USER, "closed orphaned fd=%d (handle=%d)", fd,
             handles[i]);
...
int lgNotifyOpenWithSize(int bufSize) {
  ...
  handle = lgHdlAlloc(LG_HDL_TYPE_NOTIFY, sizeof(lgNotify_t), (void **)&h,
                      _notifyClose);
  ...
  if (bufSize != 0) {
    ...
    if (i != bufSize) {
      lgHdlFree(handle, LG_HDL_TYPE_NOTIFY);
      PARAM_ERROR(LG_BAD_PATHNAME, "fcntl %s size %d failed (%m)", name,
                  bufSize);
    }
  }

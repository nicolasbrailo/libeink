...
void *lgPthTx(void) {
  ...
  while (1) {
      ...
      if (p->active) {
        if (p->next_micros <= 0) {
          if (p->type == LG_TX_PWM) {
            ...
            if (p->pulse_pos < p->num_pulses[0]) {
              xGroupWrite(p->chip, p->gpio, p->pulses[0][p->pulse_pos].bits,
    ...
    while (clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &pthTxReq, NULL))
      ;
  }
...
void lgPthTxLock(void) { pthread_mutex_lock(&lgTxMutex); }

void lgPthTxUnlock(void) { pthread_mutex_unlock(&lgTxMutex); }
...
void lgPthTxStop(lgChipObj_p chip) {
  ...
  for (pwm = txRec; pwm != NULL; pwm = pwm->next) {
    if (chip->handle == pwm->chip->handle)
      pwm->active = 0;
  }
}

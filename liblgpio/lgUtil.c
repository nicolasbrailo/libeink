...
double lguTime(void) { return (double)lguTimestamp() / (double)1E9; }
...
void lguSetWorkDir(const char *dirPath) {
  ...
  if (!xWorkDir[0]) {
    ...
    if (chdir(xWorkDir) < 0)
      LG_DBG(LG_DEBUG_ALWAYS, "can't set working directory (%s)",
             strerror(errno));
  }
}

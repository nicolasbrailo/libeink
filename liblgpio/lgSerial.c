...
int lgSerialOpen(const char *serDev, int serBaud, int serFlags) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "serDev=%s serBaud=%d serFlags=0x%X", serDev, serBaud,
         serFlags);
  ...
  handle = lgHdlAlloc(LG_HDL_TYPE_SERIAL, sizeof(lgSerialObj_t), (void **)&ser,
                      _lgSerialClose);
...
int lgSerialWriteByte(int handle, int bVal) {
  ...
  if (status == LG_OKAY) {
    ...
    if (write(ser->fd, &c, 1) != 1)
      status = LG_SERIAL_WRITE_FAILED;
...
int lgSerialDataAvailable(int handle) {
  ...
  if (status == LG_OKAY) {
    if (ioctl(ser->fd, FIONREAD, &status) == -1)
      status = 0;

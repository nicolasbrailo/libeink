...
static int xI2cSmbusAccess(int fd, char rw, uint8_t cmd, int size,
                           union lgI2cSmbusData *data) {
  struct lgI2cSmbusIoctlData args;

  LG_DBG(LG_DEBUG_INTERNAL, "rw=%d reg=%d cmd=%d data=%s", rw, cmd, size,
         lgDbgBuf2Str(data->byte + 1, (char *)data));
...
int lgI2cWriteQuick(int handle, int bit) {
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_QUICK) {
      status = xI2cSmbusAccess(i2c->fd, bit, 0, LG_I2C_SMBUS_QUICK, NULL);
...
int lgI2cReadByte(int handle) {
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_READ_BYTE) {
      status = xI2cSmbusAccess(i2c->fd, LG_I2C_SMBUS_READ, 0, LG_I2C_SMBUS_BYTE,
                               &data);
...
int lgI2cWriteByte(int handle, int bVal) {
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_WRITE_BYTE) {
      status = xI2cSmbusAccess(i2c->fd, LG_I2C_SMBUS_WRITE, bVal,
                               LG_I2C_SMBUS_BYTE, NULL);
...
int lgI2cReadByteData(int handle, int reg) {
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_READ_BYTE_DATA) {
      status = xI2cSmbusAccess(i2c->fd, LG_I2C_SMBUS_READ, reg,
                               LG_I2C_SMBUS_BYTE_DATA, &data);
...
int lgI2cWriteByteData(int handle, int reg, int bVal) {
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_WRITE_BYTE_DATA) {
      ...
      status = xI2cSmbusAccess(i2c->fd, LG_I2C_SMBUS_WRITE, reg,
                               LG_I2C_SMBUS_BYTE_DATA, &data);
...
int lgI2cReadWordData(int handle, int reg) {
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_READ_WORD_DATA) {
      status = (xI2cSmbusAccess(i2c->fd, LG_I2C_SMBUS_READ, reg,
                                LG_I2C_SMBUS_WORD_DATA, &data));

      if (status < 0) {
        LG_DBG(LG_DEBUG_USER, "error=%d (%m)", status);
        status = LG_I2C_READ_FAILED;
      } else
        status = 0xFFFF & data.word;
    } else {
...
int lgI2cWriteBlockData(int handle, int reg, const char *txBuf, int count) {
  union lgI2cSmbusData data;
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d reg=%d count=%d [%s]", handle, reg, count,
         lgDbgBuf2Str(count, txBuf));
...
int lgI2cBlockProcessCall(int handle, int reg, char *buf, int count) {
  union lgI2cSmbusData data;
...
int lgI2cReadI2CBlockData(int handle, int reg, char *rxBuf, int count) {
  union lgI2cSmbusData data;
  lgI2cObj_p i2c;
  int status;
  int i;
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d reg=%d count=%d rxBuf=%p", handle, reg,
         count, rxBuf);
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_READ_I2C_BLOCK) {
      ...
      status = xI2cSmbusAccess(i2c->fd, LG_I2C_SMBUS_READ, reg, size, &data);
...
int lgI2cWriteI2CBlockData(int handle, int reg, const char *txBuf, int count) {
  union lgI2cSmbusData data;
  ...
  if (status == LG_OKAY) {
    if (i2c->funcs & LG_I2C_FUNC_SMBUS_WRITE_I2C_BLOCK) {
      ...
      status = xI2cSmbusAccess(i2c->fd, LG_I2C_SMBUS_WRITE, reg,
                               LG_I2C_SMBUS_I2C_BLOCK_BROKEN, &data);
...
int lgI2cWriteDevice(int handle, const char *txBuf, int count) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d count=%d [%s]", handle, count,
         lgDbgBuf2Str(count, txBuf));
...
int lgI2cOpen(int i2cDev, int i2cAddr, int i2cFlags) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "i2cDev=%d i2cAddr=%d flags=0x%X", i2cDev, i2cAddr,
         i2cFlags);
  ...
  handle = lgHdlAlloc(LG_HDL_TYPE_I2C, sizeof(lgI2cObj_t), (void **)&i2c,
                      _lgI2cClose);
...
int lgI2cSegments(int handle, lgI2cMsg_t *segs, int numSegs) {
...
int lgI2cZip(int handle, const char *inBuf, int inCount, char *outBuf,
             int outCount) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d inBuf=%s outBuf=%p len=%d", handle,
         lgDbgBuf2Str(inCount, inBuf), outBuf, outCount);
  ...
  if (status == LG_OKAY) {
    ...
    while (!status && (inPos < inCount)) {
      LG_DBG(LG_DEBUG_INTERNAL,
             "status=%d inpos=%d inlen=%d cmd=%d addr=%d flags=%x", status,
             inPos, inCount, inBuf[inPos], addr, flags);

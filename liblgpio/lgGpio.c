...
static inline void xSetBit(uint64_t *b, int n) { *b |= ((uint64_t)1 << n); }

static inline void xChangeBit(uint64_t *b, int n) { *b ^= ((uint64_t)1 << n); }

static inline void xClearBit(uint64_t *b, int n) { *b &= ~((uint64_t)1 << n); }
...
static int xGpioHandleRequest(lgChipObj_p chip,
  ...
  LG_DBG(LG_DEBUG_ALLOC, "request %d with flags %llu GPIO=[%s]", req->num_lines,
         req->config.flags, lgDbgInt2Str(req->num_lines, (int *)req->offsets));
  ...
  if (status == 0) {
    ...
    LG_DBG(LG_DEBUG_ALLOC, "alloc offsets: *%p, values: *%p", (void *)offsets_p,
           (void *)values_p);
...
uint64_t xMakeFlags(uint64_t s) {
  ...
  if (s & LG_RISING_EDGE)
    f |= GPIO_V2_LINE_FLAG_EDGE_RISING;
  if (s & LG_FALLING_EDGE)
    f |= GPIO_V2_LINE_FLAG_EDGE_FALLING;
  if (s & LG_SET_ACTIVE_LOW)
    f |= GPIO_V2_LINE_FLAG_ACTIVE_LOW;
  if (s & LG_SET_OPEN_DRAIN)
    f |= GPIO_V2_LINE_FLAG_OPEN_DRAIN;
  if (s & LG_SET_OPEN_SOURCE)
    f |= GPIO_V2_LINE_FLAG_OPEN_SOURCE;
  if (s & LG_SET_PULL_UP)
    f |= GPIO_V2_LINE_FLAG_BIAS_PULL_UP;
  if (s & LG_SET_PULL_DOWN)
    f |= GPIO_V2_LINE_FLAG_BIAS_PULL_DOWN;
  if (s & LG_SET_PULL_NONE)
    f |= GPIO_V2_LINE_FLAG_BIAS_DISABLED;
  if (s & LG_SET_INPUT)
    f |= GPIO_V2_LINE_FLAG_INPUT;
  if (s & LG_SET_OUTPUT)
    f |= GPIO_V2_LINE_FLAG_OUTPUT;
...
uint64_t xMakeStatus(uint64_t f) {
  ...
  if (f & GPIO_V2_LINE_FLAG_USED)
    s |= LG_GPIO_IS_KERNEL;
  if (f & GPIO_V2_LINE_FLAG_ACTIVE_LOW)
    s |= LG_GPIO_IS_ACTIVE_LOW;
  if (f & GPIO_V2_LINE_FLAG_INPUT)
    s |= LG_GPIO_IS_INPUT;
  if (f & GPIO_V2_LINE_FLAG_OUTPUT)
    s |= LG_GPIO_IS_OUTPUT;
  if (f & GPIO_V2_LINE_FLAG_EDGE_RISING)
    s |= LG_GPIO_IS_RISING_EDGE;
  if (f & GPIO_V2_LINE_FLAG_EDGE_FALLING)
    s |= LG_GPIO_IS_FALLING_EDGE;
  if (f & GPIO_V2_LINE_FLAG_OPEN_DRAIN)
    s |= LG_GPIO_IS_OPEN_DRAIN;
  if (f & GPIO_V2_LINE_FLAG_OPEN_SOURCE)
    s |= LG_GPIO_IS_OPEN_SOURCE;
  if (f & GPIO_V2_LINE_FLAG_BIAS_PULL_UP)
    s |= LG_GPIO_IS_PULL_UP;
  if (f & GPIO_V2_LINE_FLAG_BIAS_PULL_DOWN)
    s |= LG_GPIO_IS_PULL_DOWN;
  if (f & GPIO_V2_LINE_FLAG_BIAS_DISABLED)
    s |= LG_GPIO_IS_PULL_NONE;
...
static int xClaim(lgChipObj_p chip, int lFlags, int size, const int *gpios,
                  const int *values) {
  int i;
  uint64_t m = 0, v = 0;
...
static int xSetAsFree(lgChipObj_p chip, int gpio) {
  ...
  if (GPIO->mode == LG_CHIP_MODE_UNKNOWN) {
    LG_DBG(LG_DEBUG_ALLOC, "free unallocated GPIO: %d (mode %d)", gpio,
           GPIO->mode);
  ...
  if (GPIO->mode & LG_CHIP_BIT_ALERT) {
    LG_DBG(LG_DEBUG_ALLOC, "free alert GPIO: %d (mode %d)", gpio, GPIO->mode);
  ...
  if (gpio == GPIO->offsets_p[0]) {
    LG_DBG(LG_DEBUG_ALLOC, "group free GPIO: %d (mode %d)", gpio, GPIO->mode);
...
static int xSetAsOutput(lgChipObj_p chip, int lFlags, int size,
...
static int xSetAsInput(lgChipObj_p chip, int lFlags, int size,
                       const int *gpios) {
  int mode;
...
static int xSetAsPwm(lgChipObj_p chip, int gpio, int micros_on, int micros_off,
                     int micros_offset, int cycles) {
  lgLineInf_p GPIO;
  lgTxRec_p p;
  int zero = 0;
  int status = 0;
  ...
  if (GPIO->mode == LG_CHIP_MODE_UNKNOWN) {
    xSetAsOutput(chip, GPIO_V2_LINE_FLAG_OUTPUT, 1, &gpio, &zero);
  } else {
  ...
  if (((p = lgGpioGetTxRec(chip, gpio, LG_TX_PWM)) != NULL) && p->active) {
    if (micros_on || micros_off) {
      if ((micros_on + micros_off) > lgMinTxDelay) {
        ...
        if (p->entries < LG_TX_BUF) {
          ...
          if (cycles)
            p->cycles[p->entries] = cycles;
          else
            p->cycles[p->entries] = -1;
          p->entries++;
          status = LG_TX_BUF - p->entries;
        } else
          status = LG_TX_QUEUE_FULL;
      } else
        status = LG_BAD_PWM_MICROS;
    } else {
      p->active = 0;
    }
    ...
    if ((micros_on + micros_off) > lgMinTxDelay) {
      lgGpioCreateTxRec(chip, gpio, micros_on, micros_off, micros_offset,
                        cycles);
      status = LG_TX_BUF - 1;
    } else
      return LG_BAD_PWM_MICROS;
  }

  return status;
}

static int xWave(lgChipObj_p chip, int gpio, int count, lgPulse_p pulses) {
  lgLineInf_p GPIO;
  lgTxRec_p p;
  int i;
  int zero = 0;
...
void xWrite(lgChipObj_p chip, int gpio, int value) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "chip=*%p gpio=%d value=%d", (void *)chip, gpio,
         value);
...
int lgGpiochipOpen(int gpioDev) {
  ...
  handle = lgHdlAlloc(LG_HDL_TYPE_GPIO, sizeof(lgChipObj_t), (void **)&chip,
                      _lgGpiochipClose);
...
int lgGpioSetBannedState(int handle, int gpio, int banned) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d banned=%d", handle, gpio, banned);
...
int lgGpioGetLineInfo(int handle, int gpio, lgLineInfo_p lineInfo) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d lineInfo=*%p", handle, gpio,
         (void *)lineInfo);
    ...
    if (gpio < chip->lines) {
      if (chip->LineInf[gpio].mode != LG_CHIP_MODE_UNKNOWN) {
        if (chip->LineInf[gpio].offset == 0) {
          ...
          status = LG_NOT_GROUP_LEADER;
      } else
        status = LG_GPIO_NOT_ALLOCATED;
    } else
      status = LG_BAD_GPIO_NUMBER;
...
int lgGroupClaimInput(int handle, int lFlags, int size, const int *gpios) {
  int status;
  lgChipObj_p chip;

  LG_DBG(LG_DEBUG_TRACE, "handle=%d lFlags=%x size=%d gpios=[%s]", handle,
         lFlags, size, lgDbgInt2Str(size, (int *)gpios));
...
int lgGpioClaimInput(int handle, int lFlags, int gpio) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d lFlags=%x gpio=%d", handle, lFlags, gpio);

  return lgGroupClaimInput(handle, lFlags, 1, gpios);
}

int lgGroupClaimOutput(int handle, int lFlags, int size, const int *gpios,
                       const int *values) {
  int status;
  lgChipObj_p chip;

  LG_DBG(LG_DEBUG_TRACE, "handle=%d lFlags=%x size=%d gpios=[%s] values=[%s]",
...
int lgGpioClaimOutput(int handle, int lFlags, int gpio, int value) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d lFlags=%x gpio=%d value=%d", handle, lFlags,
         gpio, value);
    ...
    if (gpio < chip->lines) {
      ...
      if (!(mode & LG_CHIP_BIT_GROUP)) {
        ...
        if (status == 0) {
          ...
          lgGpioCreateAlertRec(chip, gpio, &chip->LineInf[gpio], nfyHandle);
        } else
          status = LG_BAD_EVENT_REQUEST;
      } else
        status = LG_INVALID_GROUP_ALERT;
    } else
      status = LG_BAD_GPIO_NUMBER;
...
int lgTxPulse(int handle, int gpio, int micros_on, int micros_off,
              int micros_offset, int cycles) {
  lgChipObj_p chip;
  int status;

  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d on=%d off=%d offset=%d cycles=%d",
  ...
  if (status == LG_OKAY) {
    if (gpio < chip->lines) {
      status =
          xSetAsPwm(chip, gpio, micros_on, micros_off, micros_offset, cycles);
    } else
      status = LG_BAD_GPIO_NUMBER;
...
int lgTxPwm(int handle, int gpio, float pwmFrequency, float pwmDutyCycle,
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d freq=%f duty=%f", handle, gpio,
         pwmFrequency, pwmDutyCycle);

  if (pwmFrequency == 0.0)
    return lgTxPulse(handle, gpio, 0, 0, 0, 0);

  if ((pwmFrequency < 0.1) || (pwmFrequency > 1e4))
    PARAM_ERROR(LG_BAD_PWM_FREQ, "bad PWM frequency (%f)", pwmFrequency);

  if ((pwmDutyCycle < 0.0) || (pwmDutyCycle > 100.0))
    PARAM_ERROR(LG_BAD_PWM_DUTY, "bad PWM duty cycle (%f)", pwmDutyCycle);

  micros = ((1.0e6 / pwmFrequency) + 0.5);
  micros_on = ((pwmDutyCycle / 100.0 * micros) + 0.5);
  micros_off = micros - micros_on;

  return lgTxPulse(handle, gpio, micros_on, micros_off, pwmOffset, pwmCycles);
}
...

{
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d freq=%d width=%d", handle, gpio,
         servoFrequency, pulseWidth);

  if (pulseWidth == 0)
    return lgTxPulse(handle, gpio, 0, 0, 0, 0);

  if ((servoFrequency < 40) || (servoFrequency > 500))
    PARAM_ERROR(LG_BAD_SERVO_FREQ, "bad servo frequency (%d)", servoFrequency);

  if ((pulseWidth < 500) || (pulseWidth > 2500))
    PARAM_ERROR(LG_BAD_SERVO_WIDTH, "bad servo pulse width (%d)", pulseWidth);
  ...
  return lgTxPulse(handle, gpio, micros_on, micros_off, servoOffset,
                   servoCycles);
}
...
int lgGroupRead(int handle, int gpio, uint64_t *bits) {
  lgLineInf_p GPIO;
  int status;
  lgChipObj_p chip;
  struct gpio_v2_line_values lv;

  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d bits=%" PRIx64 "", handle, gpio,
         *bits);
...
int lgGpioSetDebounce(int handle, int gpio, int debounce_us) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d debounce_us=%d", handle, gpio,
         debounce_us);
...
int lgGpioSetWatchdog(int handle, int gpio, int watchdog_us) {
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d watchdog_us=%d", handle, gpio,
         watchdog_us);
...
int lgGpioSetAlertsFunc(int handle, int gpio, lgGpioAlertsFunc_t cbf,
  ...
  LG_DBG(LG_DEBUG_TRACE, "handle=%d gpio=%d func=*%p userdata=*%p", handle,
         gpio, cbf, userdata);
